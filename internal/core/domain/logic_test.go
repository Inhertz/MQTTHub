...
func TestRegComplete(t *testing.T) {
	...
	answer, err := domain.RegComplete(models.Sensor{ID: 1, DeviceAdd: "test"}, models.Measurement{MeasurementValue: 21, Unit: "Celcius"})
	if err != nil {
		t.Logf("expected: %v, got: %v", nil, err)
	}

	require.Equal(t, models.Measurement{MeasurementValue: 21, Unit: "Celcius", MeasurementDate: time.Now(), IDSensor: 1}, answer)
}
